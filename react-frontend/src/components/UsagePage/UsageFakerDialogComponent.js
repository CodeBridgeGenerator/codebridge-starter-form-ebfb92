...
import usageFakerFactory from "./usageFakerFactory";
...
const UsageFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = usageFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("usage").create(elem));
...
export default connect(mapState, mapDispatch)(UsageFakerDialogComponent);
