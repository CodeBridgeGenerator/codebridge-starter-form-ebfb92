...
import contentManagerFakerFactory from "./contentManagerFakerFactory";
...
const ContentManagerFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = contentManagerFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("contentManager").create(elem));
...
export default connect(mapState, mapDispatch)(ContentManagerFakerDialogComponent);
