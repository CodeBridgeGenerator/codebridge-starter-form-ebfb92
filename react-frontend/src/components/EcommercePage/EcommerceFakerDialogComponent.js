...
import ecommerceFakerFactory from "./ecommerceFakerFactory";
...
const EcommerceFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = ecommerceFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("ecommerce").create(elem));
...
export default connect(mapState, mapDispatch)(EcommerceFakerDialogComponent);
