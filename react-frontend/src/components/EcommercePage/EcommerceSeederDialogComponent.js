...
import seederData from "./ecommerce.seeder.json";
...
const EcommerceSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("ecommerce").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(EcommerceSeederDialogComponent);
