...
import protectionFakerFactory from "./protectionFakerFactory";
...
const ProtectionFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = protectionFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("protection").create(elem));
...
export default connect(mapState, mapDispatch)(ProtectionFakerDialogComponent);
