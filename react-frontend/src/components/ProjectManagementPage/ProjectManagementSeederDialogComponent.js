...
import seederData from "./projectManagement.seeder.json";
...
const ProjectManagementSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("projectManagement").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(ProjectManagementSeederDialogComponent);
