...
import projectManagementFakerFactory from "./projectManagementFakerFactory";
...
const ProjectManagementFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = projectManagementFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("projectManagement").create(elem));
...
export default connect(mapState, mapDispatch)(ProjectManagementFakerDialogComponent);
